...
`github-issues-fixes`, which could never close on your cycle, so a check that counted them
could never reach zero.

⚠ **That backlog no longer exists** (tracker wiped 2026-08-17; see the `gh_*` note above). Do
not go looking for it, and do not report its absence as a finding — a reviewer did exactly that
on 2026-08-17, correctly refusing to guess a cause, because this paragraph still described it.
The scoping rule survives the backlog on its own merits: **another branch's open issues are not
yours to count**, and a number that mixes branches tells Ozzy nothing about the one under
review.
