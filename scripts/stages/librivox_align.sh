...
docker run --rm $GPU $HF_ENV -e SONORA_ROCM_IMAGE="$IMG" \
  ...
  runuser -u ai-mgr -- bash -c 'umask 002; HF_TOKEN=\${HF_TOKEN:-} \
    python /sonora/scripts/stages/librivox_align.py \
...
if [ -x "$PY" ]; then
  "$PY" "$SONORA/scripts/stages/qc_gate.py" --campaign-dir "$OUT" || {
  ...
  echo "    .venv/bin/python scripts/tools/stage_pool.py --campaign $(basename "$OUT") --stage N --apply"
else
