#!/usr/bin/env bash
# run_in_rocm.sh — run ONE repo script inside a throwaway ROCm container.
#
# Extracted from score_holdout.sh on 2026-09-20, when a second stage
# (measure_synth_mel_error.py) needed the identical container. Copying the docker block
# would have forked the image pin, the dependency list and the argument-quoting fix across
# two files — and this repo's recurring finding is exactly that: two implementations of one
# rule drifting apart. container_env.sh says the same thing about the image itself.
#
# Usage:  run_in_rocm.sh <repo-relative script> [args...]
#   e.g.  run_in_rocm.sh scripts/stages/score_holdout.py --filelist /data/... --out /data/...
#
...
# owned by the host user, ai-mgr cannot write to it (Cython fails on core.c), and even with
# the permissions loosened this would leave build artifacts in a working tree the owner is
# editing.
set -uo pipefail

[ $# -ge 1 ] || { echo "usage: run_in_rocm.sh <repo-relative script> [args...]" >&2; exit 2; }
STAGE="$1"; shift

SONORA="$(cd "$(dirname "$0")/../.." && pwd)"
[ -f "$SONORA/$STAGE" ] || { echo "no such script in the repo: $STAGE" >&2; exit 2; }
...
docker run --rm $GPU -v /data:/data -v "$SONORA":/sonora \
  -e SONORA_ARGS="$ARGS" -e SONORA_STAGE="$STAGE" "$IMG" bash -c "
  ...
  runuser -u ai-mgr -- bash -c 'umask 002; cd /tmp/sonora && \
    ...
    eval python /tmp/sonora/\$SONORA_STAGE \$SONORA_ARGS'" || {
