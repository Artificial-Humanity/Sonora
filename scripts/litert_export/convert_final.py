...
Outputs (into ./artifacts/):
  matcha_textenc_fp16.tflite  in: emb(1,Tt,192), tmask(1,1,Tt)
                              out: mu(1,80,Tt), logw(1,1,Tt)
  matcha_decoder_fp16.tflite  in: x,mu(1,80,Tm), t_emb(1,1024), ymask(1,1,Tm)
                              out: v(1,80,Tm)
  ...
  config.json        symbols, MAX_TEXT/MAX_MEL, mel_mean/std, hop,
                     length_scale, sr

Verifies fp16 graphs end-to-end (host orchestration) vs torch reference on
several sentences -> waveform corr.
...
def main():
    """Converts the shippable fp16 artifacts and verifies e2e parity."""
    sd = B.load_sd()
    ...
    specs = [("textenc", te_r, (ex, tm0)),
             ("decoder", dec_r, (x0, mu0, te0, m0)),
             ("vocoder", gen_r, (mel0,))]
    fp16 = {}
    for name, module, inputs in specs:
        p32 = B.convert(module, inputs,
                        os.path.join(ART, f"matcha_{name}.tflite"))
        p16 = B.to_fp16(p32, os.path.join(ART, f"matcha_{name}_fp16.tflite"))
        clean = B.opcheck(p16, name + "_fp16")
        size32 = os.path.getsize(p32) / 1e6
        size16 = os.path.getsize(p16) / 1e6
        print(f"  {name}: fp32 {size32:.1f}MB -> fp16 {size16:.1f}MB  "
              f"GPU-clean={clean}")
        fp16[name] = p16
    ...
    for sentence in SENTENCES:
        ...
        seconds = len(wav_tfl) / 22050
        print(f"  corr={c:.5f}  frames={ylen}  {seconds:.2f}s  {sentence!r}")
        try:
            import soundfile as sf
            sample = os.path.join(
                ART, f"sample_{SENTENCES.index(sentence)}.wav")
            sf.write(sample, wav_tfl, 22050)
        except Exception:
            pass

    # Export host-side tables + config (the time-MLP now lives in the
    # decoder graph).
    np.save(os.path.join(ART, "emb.npy"), emb_w.numpy().astype(np.float32))
    # Raw LE f32 for Kotlin.
    ...
    cfg = dict(symbols=symbols, n_vocab=178, n_channels=192, n_feats=80,
               ...
               mel_std=mel_std, hop=256, sample_rate=22050, length_scale=0.95,
               sigma_min=1e-4, n_timesteps_default=10, time_embed_dim=1024,
               in_channels=160)
    with open(os.path.join(ART, "config.json"), "w") as f:
        json.dump(cfg, f, ensure_ascii=False, indent=2)
    print(f"\nartifacts in {ART}:")
    for name in sorted(os.listdir(ART)):
        size_mb = os.path.getsize(os.path.join(ART, name)) / 1e6
        print("  ", name, f"{size_mb:.1f}MB")
