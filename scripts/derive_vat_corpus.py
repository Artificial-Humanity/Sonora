...
vat-corpus-decision-brief.md + tension-definition-brief.md):
    ...
    T (tension slot) = phonation tension, pressed(+) <-> breathy(-):
        equal-weight sum of three per-speaker z-scored voiced-frame measures
        ...
        H1-heavy) — re-z-scored per speaker, clamped at 2 sigma. All numpy,
        no external model: zero new license surface.
    V (valence slot) = EIV-Large pseudo-labels via --valence-json
        (path -> raw score map produced by the EIV labeling run; per-speaker
        z-scored + clamped here, same recipe); 0.0 when absent.
...
def main():
    ...
    for i, (p, text, s) in enumerate(kept):
        ...
        a = clamp2(lufs_z[p])
        t = clamp2(tension_z[p])
        rows.append(f"{p}|{spk_index[s]}|{ipa}|{v:.4f},{a:.4f},{t:.4f}")
        if (i + 1) % 5000 == 0:
    ...
    report = {
        "root": args.root,
        "derivation": ("v1: A=per-speaker LUFS z@2sigma (v0, validated); "
                       "T=phonation composite z(alpha)+z(cpp)-z(h1h2), re-z@2sigma "
                       "(pressed+ / breathy-); "
                       + ("V=EIV per-speaker z@2sigma" if valence_raw else "V=0 (EIV pass pending)")),
