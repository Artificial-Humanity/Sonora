...
Run:
  .venv/bin/python scripts/lib/book_ingest.py --url <SE url> --out <dir> [--dry-run] [--max-per-type N]
"""
import argparse
import hashlib
import json
...
# #26 step 3: assets live at scripts/assets/ and sibling modules are split across the
# stages/lib/tools/gates buckets, so the anchor is the REPO ROOT. Every file under
# scripts/<bucket>/ is exactly two levels down, which is what makes this expression the
# same everywhere and checkable by tests/test_asset_paths.py.
...
def _castable_engines():
    # Deferred so importing this module does not drag `ref_select` in. `ref_select` is in
    # the same bucket (scripts/lib/) after #26 step 3, so nothing has to be added to the
    # path for this to resolve — the module-level prologue already covers it.
    import ref_select
...
# scripts/gates/test_skill_files.py.
