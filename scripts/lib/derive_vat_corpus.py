...
HOW TO RUN IT (owner decision 2026-08-01): the repo **.venv**, not `uv run`.
    ...
    .venv/bin/python scripts/lib/derive_vat_corpus.py --out data/libritts_r_vat_v3 ...
...
So: all 1,094 scored over the same 12 heads (scripts/stages/eiv_score.sh), then every derived
value rebuilt from the immutable raw scores by scripts/tools/eiv_merge_corpus.py. Pass the _v2
...
the manifest as `libritts_r` (permissive). Run from the Sonora repo root:
    python scripts/lib/derive_vat_corpus.py [--root DIR] [--out DIR] [--workers N]
...
def main():
    ...
    ap.add_argument("--homographs", action="store_true",
                    help="resolve heterophonic homographs from context (D-M4). OFF by "
                         ...
                         "the adjective. Measure first with scripts/tools/measure_homographs.py. "
    ...
    if args.homographs and args.reuse_from:
        sys.exit(
            ...
            "--reuse-from),\n"
            "   or use scripts/tools/rephonemize_corpus.py on the existing corpus.")
