...
|---|---|---|
| **`stages/`** | Pipeline stages, the four orchestrator shells that drive them, and `run_in_rocm.sh`, the container runner. Every file here is declared in `pipeline_manifest.py`, and the ratchet fails if a declared stage is not invoked *or* an invoked script is not declared. | 20 py + 5 sh |
| **`lib/`** | Imported by other non-test Python. `synth_common` (27 importers), `ref_select` (11), `book_ingest` (7)… | 17 |
| **`tools/`** | Run by hand, deliberately and repeatedly. Legitimate, and *named* so it is not mistaken for a stage. | 80 |
| **`gates/`** | Standalone gate scripts, executed as subprocesses by `tests/test_gate_scripts.py`. Named `test_*.py` and **not** collected by pytest. | 9 |
