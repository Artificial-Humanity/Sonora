...
| **`lib/`** | Imported by other non-test Python. `synth_common` (27 importers), `ref_select` (11), `book_ingest` (7)… | 12 |
| **`tools/`** | Run by hand, deliberately and repeatedly. Legitimate, and *named* so it is not mistaken for a stage. | 53 |
...
Top level holds only `pipeline_manifest.py` (the declaration) and the three container
