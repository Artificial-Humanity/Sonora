...
Usage:
    .venv/bin/python scripts/tools/stage_pool.py --campaign librivox-v1 --status
    .venv/bin/python scripts/tools/stage_pool.py --campaign librivox-v1 --stage 250 --apply
    .venv/bin/python scripts/tools/stage_pool.py --campaign librivox-v2 --stage 30 \
        --delivery-ear --apply
"""
...
def qc_flagged(campaign_dir: pathlib.Path) -> set[str]:
    ...
    if not qc.is_file():
        raise SystemExit(
            ...
            f"  Run:  .venv/bin/python scripts/stages/qc_gate.py --campaign-dir {campaign_dir}"
        )
    no_pause_measure = 0
