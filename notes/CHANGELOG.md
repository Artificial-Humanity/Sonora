...
### Fixed — `6b8df6f` `70626fc` `1f03ceb` `eb76e22`, the review findings on PR #62

Janis's review of the backlog PR itself, resolved in the PR. Both mediums were reproduced
by running the code before they were fixed, and each fix has a test that fails on its parent
commit.

- **`qc_verdict.py` — an unreadable intended label still KEPT (`6b8df6f`).**
  `intended_labels` drops an axis it cannot parse, so a row whose V/A/T were all gibberish
  reached the keep test with `checks == {}` — and `all(...)` over an empty dict is `True`.
  The clip was written to `keeps.jsonl`, which the module docstring defines as "clips +
  **confirmed** labels", with nothing confirmed; it was also counted in `undirected`,
  beneath a line claiming those rows have nothing to confirm. `--count-directed` does not
  cover this: it refuses only when **every** label in the bank is unreadable, so the
  ordinary partial case (99 good rows, one malformed) sails through the pre-flight. There
  is now a fourth outcome, `UNREADABLE`, deliberately **not** `None` — both block a keep for
  the same reason, but one means we could not measure the render and the other means the
  director's label would not parse, and the repairs differ. **Owner call, 2026-08-11: ANY
  unreadable axis blocks the keep**, not only the all-unreadable row.
- **`derive_a_channel_stats.py` — three conclusions were hardcoded strings (`70626fc`),** in
  the script written specifically to stop § 3b's numbers being assertions: § 8's "every
  named lane is quieter than `unknown` … all four the same direction", § 5's "ONE declared
  loudness target … 2 rows are `n = 1`", and § 2's "ONE lane/campaign pair is 100% in BOTH
  directions". On a probe where three of four lanes were **louder**, § 8 printed its
  sentence unchanged with the range backwards. All three now count; on the current artifact
  they print exactly what they used to say, which is the distinction — they were right and
  could not regenerate.
- **`derive_a_channel_stats.py` — § 7's `p` was a `t` on a max-minus-min (`70626fc`).** The
  spread over five lanes is a maximum over ten pairs, tested against the null of one
  pre-specified pair, so `p` was biased **low** — it overstated evidence against equal gain,
  the one direction the surrounding prose is careful about. Now a studentised range:
  **p moves 0.003 → 0.434** on the real probe, agreeing with the hedge that was already in
  the text. `probe()` also assumed a complete, balanced design; `se` is derived per
  comparison, absent cells get named refusals, and the `assert zero == 0.0` (which vanishes
  under `python -O`) is a `sys.exit`.
- **`register_audition.py` — `except ValueError` was wider than its refusal (`1f03ceb`).**
  `_build_row` also calls `os.path.relpath` and `_predicted_gender`; under `--dry-run` a
  surplus error from either was counted as `retired` and reported as "N carrying a RETIRED
  delivery lane", sending an operator to rebuild a bank over something unrelated to
  delivery. Narrowed to a `RetiredLane` subclass.
- **CI — the `pytest>=9.1` floor was not enforced by the only automated consumer
  (`eb76e22`).** The job installed bare `pytest`; it resolves ≥ 9.1 today, which means the
  day it stops being true is the day nobody notices. ⚠ `-e '.[dev]'` is **not** the fix —
  installing the project pulls `install_requires` (torch) and builds the Cython extension —
  so the floor is deliberately written twice. The comment claiming this job runs
  `scripts/test_doc_claims.py` was also wrong (`testpaths = "tests/"` does not collect it,
  and the test that would drive it skips with no `/data`), and two present-tense references
  to the retired `claude-fix.yml` were corrected.
- **`scripts/test_doc_claims.py` — the gate now sees `configs/data/*.yaml` (`eb76e22`,
  owner).** The v6 config's header is the densest corpus-number block in the repo and sat
  outside the registry entirely — the #50 failure relocated rather than closed. Measured:
  32 → 42 files, 36 → 37 recognised statements, still green. Scope alone does not enrol a
  line, so the gain is durability rather than coverage today.
- **`longcat 45` is `longcat 51` (`eb76e22`).** Flagged as uncertain from CI and settled
  against `ratings.csv`: the exclusions are 133 + 83 + 51 + 8 = 275, and 1,279 − 275 =
  **1,004** exactly, agreeing with the independent 902 labelled + 102 blank decomposition.
  The stale 45 was the unnamed residual of 6 that made this the only hop in the seven-size
  chain that did not reconcile. ⚠ **No registry FACT should guard it** — `ratings.csv` is
  written live by the Auditions app, so 1,279 moves as the audit continues.

Suite **592 passed / 9 skipped on ai-lab-0 with corpus artifacts present** (589 before).
