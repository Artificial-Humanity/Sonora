...
**`build_direction()` in `scripts/synthesis/book_ingest.py` is the single source of
...
1. **Line pass** (engine-agnostic) — `{V, A, T, register}`. `register` MUST be copied
   verbatim from `scripts/synthesis/register_lexicon.json` (47 labels, regenerated by
...
2. **Casting pass** (per engine) — `voice_design`/`instruct` governed by
   `scripts/synthesis/director_skills/<engine>.md`. The JSON schema is
...
- `scripts/synthesis/script_bank.json` — versioned authored line bank
- `scripts/synthesis/synth_<engine>.py` — renderers (`synth_common.py` = shared atomic
  wav/manifest plumbing)
- `scripts/synthesis/director_skills/<engine>.md` — the Gemma adapter per engine
- `scripts/synthesis/register_lexicon.json` + `build_register_lexicon.py`
- `scripts/synthesis/ref_select.py` — reference casting + `ENGINE_MIX_BY_LANE` +
  `SET_ASIDE` + blacklist (three-layer allocation, 2026-08-02)
- `scripts/synthesis/qc_gate.py` · `silero_vad.py` · `register_audition.py` ·
