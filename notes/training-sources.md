...
| **LibriTTS-R `train-clean-100`** | Google, CC-BY-4.0, quality-restored LibriTTS. **247** speaker dirs (⚠ read 257 until 2026-08-26, #319 — and the
`dev-clean` row below already said 247 where it mattered), 9.0 GB on disk → 31,445 rows / **51.3 h** | **TRAINED** — the only VAT source. `derisk_energy` (v1), `vat3_finetune` ep099 (v2), `vat3c_finetune` ep099 (v3c) | None outstanding. See § The VAT corpus lineage |
