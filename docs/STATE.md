...
`notes/todo.md` (private). One entry per front, newest facts first — superseded narrative is
...
> written down in `notes/quality-gap-plan.md` (private) **before** the result exists
...
> The v1–v4 lineage and every source: **`notes/training-sources.md` (private)** (SSOT).
>
...
> (`notes/training-operations.md` (private)), and run `scripts/gates/test_vat_dim_seams.py`
> (30 checks). Residual review debt that touches training: `notes/todo.md` (private).
> The quality ladder, one table: **`notes/quality-gap-plan.md` (private)**
...
> `notes/delivery-lane-remediation.md` (private). ⚠ It also **answers** the
...
verdicts, and the shortlist record: `notes/teacher-tts-audition-shortlist.md` (private);
...
`notes/delivery-mix-campaign.md` (private)).** **1,189 fold-eligible keeps**
...
measurement supports it. Full ears queue: `notes/todo.md` (private).
...
director.** Record: `notes/book-prose-lane.md` (private) § Director model.
...
- teacher-ab-v1 keeps normalized to −23 LUFS (the 5.99 dB engine spread confounded the
  Qwen-vs-VV ranking; re-test is now possible — `notes/todo.md` (private)).
...
- **Decoder v2 (DiT) is a staged spike NOW** — adopt iff it passes the parity gate vs
  ...
  `notes/matcha-siblings-study.md` (private) is the standing comparison
...
can ship. That cost belongs to the D-M4 decision (`notes/todo.md` (private)) and was invisible
when it was filed.
...
**`notes/quality-gap-plan.md` (private)** — the SSOT for sequencing. This is only its
headline; open items are in `notes/todo.md` (private).
...
2. 🔄 **Rung 3 — the 10×** (LibriTTS-R full). **BUILT 2026-08-27 · RUN 2026-08-28, stopped
   ...
   recipe and caveats: `notes/quality-gap-plan.md` (private).
...
4. **A forced-ranking pass over the 46 ceiling-tied groups** — the scale cannot separate
   six engines at 5, and the corpus trains on keeps (`notes/todo.md` (private) §4).
...
- Review sweep — 47 open items down to **9**, with **all three §1/§3 corpus decisions
  ...
  Every fix carries a regression test; the running tally is `notes/todo.md` (private).
...
- Code on `/data` — `notes/data-mirrors.md` (private). Nothing of ours is unbacked; the
