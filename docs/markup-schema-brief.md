...
|---|---|---|---|
| `utterance.vat` | continuous [−1,1] each (the contract's units: per-speaker z clamped @2σ) | the three FiLM channels, directly | instrument V/A/T within tolerance — **the value lives in `scm.VAT_TOL` and is deliberately not restated here** |
| `utterance.register` | **controlled lexicon** = `scripts/assets/register_lexicon.json` (47 labels as of 2026-07-25), regenerated by `build_register_lexicon.py` from the ratings.csv SSOT at a 3-certified-keep promotion threshold. The app remains the governance surface (labels are promoted via the Recategorize/relabel flow); the JSON is the ENFORCED projection the director must COPY from, never invent. Pre-enforcement drift, for the record: 138 distinct labels across 554 certified keeps, and 54 labels from a single 20-line director pass | nothing yet (metadata; later a register embedding if ever wanted) | EIV family profile consistency (heuristic) + owner audit |
...
1. **Freeze v0.1**: JSON Schema file + validator + inline renderer + compiler stub in
   `Sonora/scripts/tools/` (schema is a repo artifact, versioned like `bulk_spec`).
...
3. **100-clip spike** (50 certified + 50 LibriTTS-R): Gemma 26B fills the interpretive fields;
   ...
   **superseded 2026-07-28: the gate is completeness, not length — see
   `is_complete_utterance()` and scripts/gates/test_text_selection.py**. Full readout:
...
3. ✅ **Register lexicon = controlled, app-governed, and since 2026-07-25 ENFORCED IN CODE**
   via `register_lexicon.json` (promotion threshold: 3 certified keeps; off-lexicon picks are coerced).
